...
function API (options) {
  ...
  // TODO: Only map specified models.
  // app.models = Array.create(models).compact();

  app.options = Object.merge(API.defaults || {}, options || {}, true);

  return app;
}

API.args = function(req, res, next, options, done) {
  // REVISIT: Support specifying model classes to mount?
  // if (!models.length) {
  //   return next();
  // }
  ...
  args.uri = req.url.replace((options.route || '').replace('/', ''), '').split(/[\/\?]/);
  args.type = args.uri[1];
  args.ids = id(args.uri[2] || '');
  args.data;
